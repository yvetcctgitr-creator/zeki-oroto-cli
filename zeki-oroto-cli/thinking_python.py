...
def classify_defects(error_messages: List[str]) -> Dict[str, Any]:
    """Classify defects from error logs into categories for triage.
    Categories: syntax_error, import_error, type_error, assertion_failure, path_error, permission_error, network_error, unknown.
    Returns a dict with counts per category and samples.
    """
    categories = {
        "syntax_error": [],
        "import_error": [],
    ...
    for msg in error_messages or []:
        m = msg.lower()
        if "syntaxerror" in m or "invalid syntax" in m:
            categories["syntax_error"].append(msg)
        elif "module not found" in m or "importerror" in m or "no module named" in m:
            categories["import_error"].append(msg)
        elif "typeerror" in m or "is not iterable" in m or "none type" in m:
            categories["type_error"].append(msg)
        elif "assert" in m or "assertion" in m or "failed" in m and "test" in m:
            categories["assertion_failure"].append(msg)
        elif "path" in m and ("not found" in m or "traversal" in m or "outside" in m):
            categories["path_error"].append(msg)
        elif "permission" in m or "denied" in m:
            categories["permission_error"].append(msg)
        elif "http" in m or "network" in m or "timeout" in m:
            categories["network_error"].append(msg)
        else:
            categories["unknown"].append(msg)
    summary = {k: len(v) for k, v in categories.items()}
    return {"summary": summary, "samples": categories}
